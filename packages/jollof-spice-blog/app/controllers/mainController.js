...
exports.index = async (ctx) => {
    ...
    ctx.state.articles = await jollof.models.Article.find('', {raw:true});
...
exports.article = async (ctx) => {
    ctx.state.article = await jollof.models.Article.findById(ctx.params.id,{raw:true});
...
exports.category = async (ctx) => {
    const category = await jollof.models.Category.findById(ctx.params.id);

    ctx.state.articles = await jollof.models.Article.find(jql`category = ${ctx.params.id}`);
